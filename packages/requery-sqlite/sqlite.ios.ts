...
function eachRaw(db: FMDatabase, query: string, params: SqliteParams, asObject: boolean, callback: (error: Error, result: SqliteRow | SqliteParam[]) => void, complete: (error: Error, count: number) => void, transformBlobs?: boolean) {
  // const statement = prepareStatement(db, query);
  // const cursorSt = getNewCursorStatement(statement);
  // bind(params, statement);
  // let rows = [];
