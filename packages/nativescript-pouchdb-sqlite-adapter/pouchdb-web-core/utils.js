...
function compactRevs(revs, docId, tx) {
      ...
      db.execute(sql, seqs).then(() => {
        ...
        db.select(sql, digestsToCheck).then((res) => {
          ...
          digestsToCheck.forEach(function (digest) {
            ...
            db.execute('DELETE FROM ' + ATTACH_AND_SEQ_STORE + ' WHERE digest=?', [digest]);
            db.execute('DELETE FROM ' + ATTACH_STORE + ' WHERE digest=?', [digest]);
          });
        });
      });
    });
  ...
  revs.forEach(function (rev) {
    ...
    db.select(sql, [docId, rev]).then((res) => {
      ...
      db.execute('DELETE FROM ' + BY_SEQ_STORE + ' WHERE seq=?', [seq]).then(checkDone);
    });
  });
}
...
export { escapeBlob, unescapeBlob, stringifyDoc, unstringifyDoc, qMarks, select, compactRevs, getSize, websqlError, uuid };
