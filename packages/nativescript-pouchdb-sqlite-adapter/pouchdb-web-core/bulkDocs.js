...
import { select, stringifyDoc, compactRevs, websqlError, escapeBlob } from './utils';
...
function websqlBulkDocs(dbOpts, req, opts, api, db, websqlChanges, callback) {
  ...
  function writeDoc(docInfo, winningRev, winningRevIsDeleted, newRevIsDeleted, isUpdate, delta, resultsIdx, callback) {
    function finish() {
      ...
      db.execute(sql, sqlArgs)
        .then(
          db.select('SELECT last_insert_rowid()').then((res) => {
  ...
  function websqlProcessDocs() {
    processDocs(dbOpts.revs_limit, docInfos, api, fetchedDocs, db, results, writeDoc, opts);
  }
  ...
  function fetchExistingDocs(callback) {
    ...
    docInfos.forEach(function (docInfo) {
      ...
      var id = docInfo.metadata.id;
      db.select('SELECT json FROM ' + DOC_STORE + ' WHERE id = ?', [id]).then((result) => {
        if (result.length) {
  ...
  function saveAttachment(digest, data, callback) {
    ...
    db.select(sql, [digest]).then((result) => {
      ...
      db.execute(sql, [digest, escapeBlob(data)])
        .then(() => {
          callback();
        })
        .catch(function () {
